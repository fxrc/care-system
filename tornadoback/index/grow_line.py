...
import pandas as pd
import tornadoback.escape
from judge_permission import judgeIfPermiss
from api_define import grow_line
from orm import *

class GrowLine(grow_line):
    ...
    def getData(self, user_name):
        ...
        #先拿到所有学院的名称集合
        college_list = MyBaseModel.returnList(school_college_info.select(school_college_info.college).dicts(), key = "college")

        res["date"] = list(data_pd.groupby(by = ["focusDate"]).count().index)
        res["全校学生"] = data_pd.groupby(by = ["focusDate"]).count()["stuID"].tolist()

        college_data = data_pd.groupby(by=["focusDate", "collegeid"])["stuID"].count().unstack().fillna(0).to_dict("list")
