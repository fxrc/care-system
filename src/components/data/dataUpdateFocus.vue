<template>
    <div>
        ...
        <el-row style="margin-top:30px" type="flex" justify="center">
            <el-col :span="6">
                <el-upload drag :action="dataUpdateFocusUrlT" multiple show-file-list name="file" :on-success="funGetSuccessResponse" :on-error="funGetErroeResponse" :before-upload="funBeforeUpload">
...
<script>
import { dataGetModeFocus, JSONToExcelConvertor, dataGetFocusFailed } from '@/api/api'
import { dataUpdateFocusUrl, dataUpdateFocusUrlGetModel } from '@/api/httpapi'

export default {
    data() {
        return {
            dialogVisible: false,
            dataUpdateFocusUrlT: dataUpdateFocusUrl,
            loadingInstance: ''
        }
    },
    methods: {
        downLoad() {
            window.location = dataUpdateFocusUrlGetModel
            return ''
        },
        ...
        funGetSuccessResponse(response, file, fileList) {
            ...
            if (response['status'] == 0) {
                ...
                dataGetFocusFailed(response["file"])
                this.$notify({
                    title: '未全部完成',
