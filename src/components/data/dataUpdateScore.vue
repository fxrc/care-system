<template>
    <div>
        ...
        <el-row style="margin-top:30px" type="flex" justify="center">
            <el-col :span="6">
                <el-upload drag :action="dataUpdateScoreUrlT" multiple show-file-list name="file" :on-success="funGetSuccessResponse" :on-error="funGetErroeResponse" :before-upload="funBeforeUpload">
        ...
        <el-dialog title="使用说明" :visible.sync="dialogVisible" size="tiny">
            ...
            <span class="txt">您上传的数据将以课程号与课序号为组合关键字进行覆盖/新增操作,即:若该组合关键字已存在于当前数据库中,系统将依据您最新上传的数据更新旧的数据;若该组合关键字并不存在于当前的数据库中,系统将数据新增入库。</span>
            </br>
...
<script>
import { officeSuggestions, dataGetModeScore, JSONToExcelConvertor, dataGetScoreFailed } from '@/api/api'
import { dataUpdateScoreUrl, dataUpdateScoreUrlGetModel } from '@/api/httpapi'

export default {
    data() {
        return {
            dialogVisible: false,
            dataUpdateScoreUrlT: dataUpdateScoreUrl,
            loadingInstance: ''
        }
    },
    methods: {
        downLoad() {
            window.location = dataUpdateScoreUrlGetModel
            return ''
            var model = dataGetModeScore()
            var title = []
            var data = []
            for (var con in model) {
                title.push([con])
            }
            JSONToExcelConvertor(data, "学生关怀系统--成绩数据导入模板", title)
        },
        ...
        funGetSuccessResponse(response, file, fileList) {
            ...
            if (response['status'] == 0) {
                ...
                dataGetScoreFailed(response["file"])
                this.$notify({
                    title: '未全部完成',
