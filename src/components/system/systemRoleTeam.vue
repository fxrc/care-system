    ...
    <div>
        <el-row style="margin-top:50px;padding:10px 10px" :gutter="40">
            ...
            <el-col :span="3" :offset="5">
                <el-tooltip effect="light" content="添加角色" :enterable="false" :open-delay="300" placement="left">
        ...
        <el-row style="margin-top:30px">
            <el-col :span='20' :offset='2'>
                    ...
                    <el-table-column label="操作" align='center'>
                        <template slot-scope="scope">
                            <el-row>
                                ...
                                <el-col :span="2" :offset="2">
                                    <el-tooltip content="删除该角色组" placement="left-start" effect="light" :enterable="false" :open-delay="400" transition="el-zoom-in-center">
                ...
                <el-dialog :title="changePowerTitle" :visible.sync="eventDialogFormVisible" size='tiny' @open="initChangePowerCheckList">
                    <el-row>
                        <el-col :span="20" :offset="2">
                            <el-checkbox-group v-model="changePowerCheckList">
                                <template v-for="(item, index) in changePowerData['data']">
                                    <el-checkbox :label="$store.state.pageNameLower[index]" :key="index" align='center' style="margin-top:10px;margin-left:10px">
                                    </el-checkbox>
                                </template>
                            </el-checkbox-group>
                        </el-col>
                ...
                <el-dialog title="提示" :visible.sync="delRoleTeamDialogVisible" size="tiny">
                    ...
                    <span slot="footer" class="dialog-footer">
                        <el-row>
                            <el-col :span="18" :offset="3">
                                <div style="display: flex;float:right">
                                    <el-button @click="ifDelRoleTeam(0)">取 消</el-button>
                                    <el-button type="primary" @click="ifDelRoleTeam(1)">确 定</el-button>
                                </div>
                            </el-col>
                        </el-row>
                    </span>
...
<script>
import { systemGetTotalRoleTeam, systemGetOneRoleTeam, systemSetOneRoleTeam, systemDelOneRoleTeam, systemAddOneRoleTeam } from '@/api/api'
...
export default {
    data() {
        return {
            ...
            uploadCenter: "fa fa-vcard fa-5x txt-blue-upload",
            rulesForAddPower: {
                name: [
    ...
    methods: {
        ...
        handleEdit(index, row, name) {
            ...
            if (name == 'Change') {
                let that = this
                systemGetOneRoleTeam(this.$store.state.userid, row['name']).then(
                    (data) => {
            ...
            if (mode == 0) {
                ...
                this.$refs[this.addPowerData].validate((valid) => {
                    if (valid) {
                        //添加一个角色组
                        let that = this
                        systemAddOneRoleTeam(this.$store.state.userid, this.addPowerData).then(
                            (data) => {
        ...
        funUploadCenter(mode) {
            ...
            //mode为1是鼠标移动开了
            if (mode == 0) {
                this.uploadCenter = "fa fa-vcard fa-5x txt-green-upload"
            } else if (mode == 1) {
                this.uploadCenter = "fa fa-vcard fa-5x txt-blue-upload"
            }
        },
        getReversalPageName(value){
            //给中文，返回小写的页面名
            var item
            for (item in this.$store.state.pageNameLower){
                if (this.$store.state.pageNameLower[item] == value)
                {
                    return item
                }
            }
        ...
        changePower(mode) {
            ...
            if (mode == 0) {
                ...
                for (index = 0; index < this.changePowerCheckList.length; index++) {
                    this.changePowerData['data'][this.getReversalPageName(this.changePowerCheckList[index])] = 1
                }
                let that = this
                systemSetOneRoleTeam(this.$store.state.userid, this.rowName, this.changePowerData['data']).then(
                    (data) => {
        ...
        initChangePowerCheckList() {
            ...
            for (var item in this.changePowerData['data']) {
                if (this.changePowerData['data'][item].toString() == "1") {
                    this.changePowerCheckList.push(this.$store.state.pageNameLower[item])
                }
            }
            //console.log(this.changePowerCheckList)
        },
        ifDelRoleTeam(mode) {
            ...
            if (mode == 0) {
                ...
                let that = this
                systemDelOneRoleTeam(this.$store.state.userid, this.rowName).then(
                    (data) => {
                        if (data['status'] == 1) {
                            that.initTable()
                            that.$notify({
